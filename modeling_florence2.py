...
class Florence2FlashAttention2(Florence2Attention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            raise ValueError(
                "Florence2FlashAttention2 attention does not support `output_attentions` or `layer_head_mask`. "
...
class Florence2SdpaAttention(Florence2Attention):
    def forward(
        ...
        if layer_head_mask is not None:
            raise ValueError(
                "Florence2SdpaAttention attention does not support `output_attentions` or `layer_head_mask`. "
                "Use the argument `attn_implementation='eager'` when loading the model."
            )
...
class Florence2Encoder(Florence2LanguagePreTrainedModel):
    ...
    def __init__(self, config: Florence2LanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([Florence2EncoderLayer(config, layer_idx=i) for i in range(config.encoder_layers)])
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def __init__(self, config: Florence2LanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([Florence2DecoderLayer(config, layer_idx=i) for i in range(config.decoder_layers)])
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self._use_sdpa = config._attn_implementation == "sdpa"
